...
class VM_RotatedConvFCBBoxHead(RotatedBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(VM_RotatedConvFCBBoxHead, self).__init__(
        ...
        if not os.path.exists(save_path):
            ...
            for epoch in range(20):
                ...
                loss = cosine_similarity_loss(vectors)
            ...
            print(f"Final trained angle matrix saved to {save_path}")
    ...
    def forward(self, x):
        ...
        # Classification convolution and FC layers
        ...
        angle_score = 100.0 * (image_feats @ angle_feats.T)

        # Regression convolution and FC layers
        ...
        return angle_score, bbox_pred


@ROTATED_HEADS.register_module()
class VM_RotatedShared2FCBBoxHead(VM_RotatedConvFCBBoxHead):
    """Shared2FC RBBox head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(VM_RotatedShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
...
@ROTATED_HEADS.register_module()
class VM_RotatedShared4Conv1FCBBoxHead(VM_RotatedConvFCBBoxHead):

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(VM_RotatedShared4Conv1FCBBoxHead, self).__init__(
            num_shared_convs=4,
            num_shared_fcs=1,
            num_cls_convs=0,
            num_cls_fcs=0,
...
@ROTATED_HEADS.register_module()
class VM_RotatedKFIoUShared2FCBBoxHead(VM_RotatedConvFCBBoxHead):
    """KFIoU RoI head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(VM_RotatedKFIoUShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
